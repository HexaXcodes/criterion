...
// Lazy load or conditional setup for transporter
...
if (!process.env.RESEND_API_KEY) {
  ...
  transporter.verify((error) => {
    if (error) {
      console.log("Email service error (Nodemailer):", error.message);
    } else {
...
/**
 * Common helper to send emails via Resend or Nodemailer
 */
const sendEmail = async (to, subject, html) => {
  const fromName = process.env.EMAIL_FROM_NAME || "MovieReview Team";
...
exports.sendWelcomeEmail = async (email, name) => {
  try {
    ...
    await sendEmail(email, "🎬 Welcome to MovieReview!", html);
...
exports.sendPasswordResetEmail = async (email, name, code, resetLink) => {
  try {
    ...
    await sendEmail(email, "🔐 Password Recovery - MovieReview", html);
...
exports.sendPasswordChangedEmail = async (email, name) => {
  try {
    ...
    await sendEmail(email, "✅ Password Changed Successfully - MovieReview", html);
